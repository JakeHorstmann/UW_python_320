...
class TestMainUserData(unittest.TestCase):
    ...
    def test_init_database(self):
        ...
        """
        Test for main's init_database function
        """
        # replace the user collection with a test value
        with patch("socialnetwork_model.get_ds") as mock:
    ...
    def test_load_users_good_data(self):
        ...
        with (patch("verify_input.verify_csv_file", return_value = [True, ""]),
              patch("users.load_users", return_value = Mock(return_value = True))):
            # use mock user collection tool
            response = main.load_users("foo", self.db)
    ...
    def test_load_users_bad_csv_data(self):
        ...
        """
        # replace open so that it does not error out when called with fake file
        with (patch("users.load_users", return_value = Mock(return_value = True)),
    ...
    def test_load_users_bad_path(self):
        ...
        with patch("verify_input.verify_csv_file", return_value = [False, ""]):
            response = main.load_users(test_path, self.db)
            self.assertEqual(response, False)
    ...
    def test_add_user_new(self):
        ...
        with (patch("verify_input.verify_user_id", return_value = [True, ""]),
              ...
              patch("main.search_user", return_value = False),
              patch("users.add_user", return_value = Mock(return_value = True))):
    ...
    def test_add_user_bad_id(self):
        ...
        mock_user_collection = Mock()
        # call the add_user from main and get response
        with patch("verify_input.verify_user_id", return_value = False):
    ...
    def test_update_user_valid_id(self):
        ...
        with (patch("verify_input.verify_user_id", return_value = [True, ""]),
              ...
              patch("verify_input.verify_user_name", return_value = [True, ""]),
              patch("verify_input.verify_user_last_name", return_value = [True, ""]),
              patch("main.search_user", return_value = True),
              patch("users.update_user", return_value = Mock(return_value = True))):
    ...
    def test_delete_user_valid(self):
        ...
        with (patch("verify_input.verify_user_id", return_value = [True, ""]),
              patch("main.search_user", return_value = False),
              patch("users.delete_user", return_value = Mock(return_value = True)),
              patch("users.delete_status", return_value = Mock(return_value = True))):
            # delete user from database
            response = main.delete_user("testuser", self.db)
