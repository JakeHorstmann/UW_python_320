...
def add_user(user_id, user_email, user_name, user_last_name, table):
    ...
    return add_user_inner(user_id=user_id, user_email=user_email, user_name=user_name, user_last_name=user_last_name)
...
def delete_user(user_id, table_users, table_status):
    '''
...
def save_all_pictures(table):
    ...
    for pic in pics:
        ...
        file_path = path.join(folder_path, f"{id}.png")
        # create file
...
def reconcile_pictures(table):
    ...
    for pic in pictures.get_all_pictures(table):
        id = pic["picture_id"]
        tags = deconstruct_tags(pic["tags"])
        pic_path = path.join("pictures", pic["user_id"], *tags, id + ".png")
        db_pics_data[id] = pic_path
    ...
    missing_from_db = [(pic_path, pic_id) for pic_id, pic_path in local_pics_data.items() if pic_id not in db_pics_data.keys()]
    missing_from_local = [(pic_path, pic_id) for pic_id, pic_path in db_pics_data.items() if pic_id not in local_pics_data.keys()]
