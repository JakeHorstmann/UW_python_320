...
def load_users():
    ...
    main.load_users(filename, user_collection, batch_size = 999//4)
...
def load_status_updates():
    ...
    main.load_status_updates(filename, status_collection, batch_size = 999//3)
...
def search_all_status_updates():
    ...
    """
    user_id = input("User ID: ")
    results = main.search_all_status_updates(user_id, status_collection)
    if results:
        # find the length of the results returned
        result_len = len(list(results))
        # determine is the sentence needs plural forms or not
        plural_status = "" if result_len == 1 else "s"
        ...
        print(f"A total of {result_len} statuse{plural_status} {was_or_were} found for {user_id}")
...
def flagged_status_updates():
    ...
    for status_tuple in [(result.status_id, result.status_text) for result in results]:
        print(status_tuple)
