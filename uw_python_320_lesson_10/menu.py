...
def delete_user():
    ...
    '''
    user_id = input('User ID: ')
    if main.delete_user(user_id, table_users=user_table,
                        table_status=status_table, table_pictures=picture_table):
...
def get_tags():
    ...
    while True:
        ...
        if tag_len > 101:
            print("You have reached the tag character limit.\
                  Your last tag will be dropped off to fit the limit.")
            break
...
def reconcile_pictures():
    ...
    if missing_from_db:
        ...
        for pic in missing_from_db:
            print(f"Picture {pic['picture_id']} with path \
                  {pic['picture_path']} is missing from the database")
    else:
        print("!!! NO LOCAL FILES MISSING FROM DB !!!")
    if missing_from_local:
        print("*** MISSING FROM LOCAL ***")
        for pic in missing_from_local:
            print(f"Picture {pic['picture_id']} with path \
                  {pic['picture_path']} is missing from local files")
    else:
        print("!!! NO DATABASE FILES MISSING FROM LOCAL !!!")
